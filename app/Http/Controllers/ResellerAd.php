...
use App\Models\ResellerAds;
use App\Models\AdminAds;

class ResellerAd extends Controller
{
    public function index()
    {
        return view('admin.reseller_ads.index');
    }

    /* Process ajax request */
    public function getResellerAdsList(Request $request)
    {
        ...
        // Total records
        // Total records
        $totalRecords = ResellerAds::select('count(*) as allcount')->whereNull('reseller_ads.deleted_at')->where('user_id', \Auth::user()->id)->count();
        $inactiveRecords = ResellerAds::select('count(*) as allcount')->where('status','0')->whereNull('reseller_ads.deleted_at')->where('user_id', \Auth::user()->id)->count();
        $activeRecords = ResellerAds::select('count(*) as allcount')->where('status','1')->whereNull('reseller_ads.deleted_at')->where('user_id', \Auth::user()->id)->count();
        $deletedRecords = ResellerAds::select('count(*) as allcount')->whereNotNull('reseller_ads.deleted_at')->where('user_id', \Auth::user()->id)->count();


        $totalRecordswithFilter = ResellerAds::select('count(*) as allcount')
        ->where(function ($query) use ($searchValue) {
            $query->whereNull('reseller_ads.deleted_at')
                ->where('reseller_ads.user_id', \Auth::user()->id)
                ->where('reseller_ads.title', 'like', '%' . $searchValue . '%');
        })
        ->count();

        // Get records, also we have included search filter as well
        $records = ResellerAds::orderBy($columnName, $columnSortOrder)
            // ->where('reseller_ads.status', '=', 1)
            ->whereNull('reseller_ads.deleted_at')
            ->where('reseller_ads.title', 'like', '%' . $searchValue . '%')
            ->where('reseller_ads.user_id', \Auth::user()->id)            
            ->select('reseller_ads.*')
            // ->leftJoin('plans', 'plans.id', '=', 'ResellerAds.ResellerAds_id')
            ->skip($start)
            ->take($rowperpage)
            ->get();
    ...
    public function edit($id){                
        $id = base64_decode($id);
        $this->data['super_admin_ads'] = AdminAds::whereNull('deleted_at')->get();
        $this->data['ad_plan'] = ResellerAds::where('id', $id)->first();
        return view('admin.reseller_ads.add', $this->data);
    }
    ...
    public function save(Request $request){
        ...
        $admin_ad_plan = !empty($request->id) ? ResellerAds::firstwhere('id',$request->id) : new ResellerAds();                       
    ...
    public function getAdDetails(Request $request){
        $id = $request->id;
        $admin_ad = AdminAds::where('id', $id)->first();
        if ($admin_ad) {
    ...
    public function destroy(Request $request){
        $ad_plan = ResellerAds::where('id',base64_decode($request->id))->first();
    ...
    public function updateStatus($id){
        $ad_plan = ResellerAds::find(base64_decode($id));
        if($ad_plan){
