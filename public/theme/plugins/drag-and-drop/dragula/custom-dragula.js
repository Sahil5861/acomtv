...
var drake = dragula([$('left-defaults'), $('right-defaults')],{moves: function (el, container, handle) {
        if (el.classList.contains('no-move')) {
            return false;
        }
    ...
    // ,isContainer: function (el, container, handle) {
...
//alert('j')
  ...
  setTimeout(()=>{
    var i = 1;
    Array.from(document.getElementsByClassName("c-index")).forEach(function(item) {
       // console.log(item.id);
...
setTimeout(()=>{
    var new_channel_no = Array.from(el.parentNode.children).indexOf(el) + 1;
    var channel_no = el.id.split('_')[2];
    document.getElementById("new_channel_no").value = new_channel_no;
    document.getElementById("old_channel_no").value = channel_no;
    if(new_channel_no > channel_no){
      document.getElementById("checkOrder").value = channel_no;
      // checkOrder
    }
    console.log(new_channel_no)
    console.log(channel_no)
    console.log(document.getElementById("lockedChannels").value);
...
var drake2 = dragula([$('left-defaults-1'), $('right-defaults-1')],{})
.on('drag', function (el) {
...
if(autoScroll && document.querySelector('#_dragula2')){
  var scroll2 = autoScroll([
          window,
          document.querySelector('#_dragula2')
      ],{
      margin: 20,
      autoScroll: function(){
          return this.down && drake2.dragging;
      }
  });
}


if(autoScroll && document.querySelector('#_dragula')){
  var scroll = autoScroll([
          window,
          document.querySelector('#_dragula')
      ],{
      margin: 20,
      autoScroll: function(){
