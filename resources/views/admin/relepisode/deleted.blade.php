...
@section('mytitle', 'Deleted Channel')
@section('page', 'Channels  /  Deleted Channel')
        ...
        <div class="col-xl-3 col-lg-6 col-md-6 col-sm-6 col-12 layout-spacing">
            <div class="widget widget-card-four">
                <div class="widget-content">
                    <div class="w-content">
                        <div class="w-info">
                            <p class=""><small>Total Deleted Channels</small></p>
  ...
  <script type="text/javascript">
    $(document).ready(function(){
      ...
      $('#multi-column-ordering').DataTable({
         processing: true,
         serverSide: true,
         ajax: "{{route('getDeletedChannelList')}}",
         columns: [
            { data: 'channel_name' },
            { data: 'channel_number' },
            // { data: 'channel_logo' },
        ...
        swal.mixin({
            ...
            if (result.value != '') {
                $.ajax({
                    ...
                    data: {
                        'id': id,
                        'channel_number': result.value[0],
                        "_token": "{{ csrf_token() }}"
                    },
                    success: function(response){
                        if(response){
                            url = new URL(window.location.href).origin +'/recover-channel/'+btoa(id);
                            window.location.href =url;
                        }else{
                            swal({
                                title: 'Channel number already used',
                                animation: true,
                ...
                swal({
                    title: 'Channel number is mandatory',
                    animation: true,
                    customClass: 'animated tada',
                    padding: '2em'
                })
